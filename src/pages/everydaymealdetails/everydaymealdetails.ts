...
/**
 * Generated class for the EverydaymealdetailsPage page.
 *
...
@IonicPage({})
@Component({
  selector: 'page-everydaymealdetails',
  templateUrl: 'everydaymealdetails.html',
})
export class EverydaymealdetailsPage {
  ...
  constructor(public http: Http, public navCtrl: NavController, public navParams: NavParams, public storage: Storage, public alertCtrl: AlertController) {
  	this.everydaymeal = this.navParams.get("everydaymeal");
              this.savedMeal = [{
            breakfast : false,
            lunch : false,
            dinner : false
...
updateProgressBar(){
  this.loadProgress = 0;
  this.progress=0;
  if(this.savedMeal[this.everydaymeal.id].breakfast){
    this.progress++;
  }
  if(this.savedMeal[this.everydaymeal.id].lunch){
    this.progress++;
  }
  if(this.savedMeal[this.everydaymeal.id].dinner){
    this.progress++;
  }
  ...
  updateMeal(meal){
    ...
    if (meal==3)
     ...
     console.log(this.savedMeal);
        ...
        audittrail.push({
          name : "Day "+this.everydaymeal.id+": Marked "+_meal,
     ...
     if(this.everydaymeal.id==7 && this.progress==3){
