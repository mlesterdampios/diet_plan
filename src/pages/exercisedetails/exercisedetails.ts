...
export class ExercisedetailsPage {
    ...
    this.storage.ready().then(()=>{
      this.storage.get("userLoginInfo").then((userLoginInfo)=>{
        if(userLoginInfo != null){
          ...
          this.loggedIn = true;
          /*
          this.http.get("http://192.168.1.7/diet_app/Api/isUserHaveExercised/"+this.user.id+"/"+this.exercise.id).subscribe ((res)=>{
  ...
  RecordAsDone(){
    ...
    this.http.post("http://192.168.1.7/diet_app/Api/UserHaveExercised/", postData )
