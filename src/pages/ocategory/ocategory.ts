...
/**
 * Generated class for the oCategoryPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-ocategory',
  templateUrl: 'ocategory.html',
})
export class oCategoryPage {
  ...
  constructor(public storage: Storage, public loadingCtrl: LoadingController,public navCtrl: NavController, public navParams: NavParams,public http: Http, public toastCtrl: ToastController) {
   ...
   if(this.user.bmi_mode==3){
      for(var i=0; i < this.ocategorys.length; i++){
         this.ocategorys[i].profile_img = "assets/images/obese"+(i+1)+".png";
      }
   }
   if(this.user.bmi_mode==2){
      for(var i=0; i < this.ocategorys.length; i++){
         this.ocategorys[i].profile_img = "assets/images/well-fitted"+(i+1)+".png";
      }
   }
   if(this.user.bmi_mode==1){
      for(var i=0; i < this.ocategorys.length; i++){
         this.ocategorys[i].profile_img = "assets/images/malnourished"+(i+1)+".png";
      }
   }
  }else{
  ...
  openExerciseByCat(ocategory){
    this.navCtrl.push('oExercisePage', { "cat":ocategory });
  }
}
