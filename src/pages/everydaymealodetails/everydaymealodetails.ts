...
/**
 * Generated class for the EverydaymealodetailsPage page.
 *
...
@IonicPage({})
@Component({
  selector: 'page-everydaymealodetails',
  templateUrl: 'everydaymealodetails.html',
})
export class EverydaymealodetailsPage {
  ...
  constructor(public http: Http, public navCtrl: NavController, public navParams: NavParams, public storage: Storage, public alertCtrl: AlertController) {
  	this.everydaymealo = this.navParams.get("everydaymealo");
              this.savedMeal = [{
            breakfast : false,
            lunch : false,
            dinner : false
  	...
  	console.log(this.everydaymealo);
  }
/*
  savedMeal = [{
    breakfast : true,
...
updateProgressBar(){
  this.loadProgress = 0;
  this.progress=0;
  if(this.savedMeal[this.everydaymealo.id].breakfast){
    this.progress++;
  }
  if(this.savedMeal[this.everydaymealo.id].lunch){
    this.progress++;
  }
  if(this.savedMeal[this.everydaymealo.id].dinner){
    this.progress++;
  }
  ...
  updateMeal(meal){
    ...
    if (meal==3)
     ...
     console.log(this.savedMeal);
        ...
        audittrail.push({
          name : "Day "+this.everydaymealo.id+": Marked "+_meal,
     ...
     if(this.everydaymealo.id==7 && this.progress==3){
